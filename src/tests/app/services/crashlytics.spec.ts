...
describe('Crashlytics', () => {
  ...
  describe('with a window.fabric instance', () => {
    beforeEach(() => {
      this.fabricInstance = {foo: 'bar'};
      (window as any).fabric = this.fabricInstance;
      this.crashlytics = new Crashlytics(new PlatformMock() as Platform);
    });

    afterEach(() => {
      (window as any).fabric = undefined;
    });

    it('set the handler to the window.fabric instance', (done) => {
      setTimeout(
        () => { // wait for Platform.ready to fire
          expect(this.crashlytics["handler"]).toEqual(this.fabricInstance);
          done();
        },
        50);
    });
  ...
  describe('without any window.fabric instance', () => {
    beforeAll(() => {
      this.crashlytics = new Crashlytics(new PlatformMock() as Platform);
    });
